...
###configuration 
my $mutect="/home/users/xu/mutect-src/mutect/target/mutect-1.1.7.jar";
my $tumor_dir="/home/proj/MDW_genomics/xu/final_bam/";
my $normal_dir="/home/proj/MDW_genomics/xu/final_bam/";
my $output_dir="/scratch/xu/MDV_project/mutect_results";
...
foreach my $num (10..25){
	...
	my $cmd="java -jar $mutect --analysis_type MuTect  --reference_sequence $genome --input_file:tumor $tumor_bam --input_file:normal $normal_bam --out  $output_dir/$sample.classical.out --coverage_file $output_dir/$sample.cov.out --vcf $output_dir/$sample.vcf ";
	print "$cmd\n";
	`qsub -b y -q all.q -l vf=36G,core=1  -N "mutect$sample" "$cmd"`;
