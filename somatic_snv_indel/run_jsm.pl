...
foreach my $num (0..0){
    ...
    $tumors[$num]=~/.*(S\d+)/;
    my $sample=$1;
    #this $cmd1 only for sample 798-1_S5, see README.md file in the same directory for details
    ...
    my $cmd1="$jsm train joint_snv_mix_two --min_normal_depth 8 --min_tumour_depth 6  $genome $normal_bam $tumor_bam $config/joint_priors.cfg $config/joint_params.cfg $output_dir/$sample.cfg ";
    ...
    my $cmd3=qq(awk -F \"\\t\" 'NR!=1 && \$4!="N" && \$10+\$11>=0.95' $output_dir/$sample.tsv >$output_dir/$sample.filtered.tsv);
    print OUT "$cmd1\n$cmd2\n$cmd3\n";
    close OUT;
