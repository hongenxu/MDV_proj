...
my $somaticseq="/home/users/xu/somaticseq-2.1.2";
...
my $ref_dict="/home/proj/MDW_genomics/xu/galgal5/galgal5.dict";
...
my $lofreq_dir="$main_in_dir/lofreq_results";
####output directory
...
####################lofreq###################
#does not require modification. It has no sample columns anyway.
print "cp $lofreq_dir/$sample\_somatic_final.indels.vcf $output_dir/$sample.lofreq.indel.vcf";
print "\n";
...
my @indel_callers=("indelocator","varscan2","vardict","lofreq");
...
print "java -jar $gatk -T CombineVariants -R $genome --setKey null --genotypemergeoption UNSORTED -V $output_dir/$sample.indelocator.indel.vcf -V $output_dir/$sample.varscan2.indel.vcf  -V $output_dir/$sample.vardict.indel.vcf -V $output_dir/$sample.lofreq.indel.vcf --out $output_dir/$sample.combined.indel.vcf\n";
print "\n";
...
#this step was deprected in version 2.1.2
...
#print "$python $somaticseq/score_Somatic.Variants.py  -tools CGA VarScan2 VarDict LoFreq -infile $output_dir/$sample.EFF.dbSNP.combined.indel.vcf  -mincaller 1 -outfile $output_dir/$sample.bina.somatic.indel.vcf\n";
#print "\n";

#####################Convert the VCF file and annotate ##########
print "$python $somaticseq/SSeq_merged.vcf2tsv.py -ref $genome -myvcf $output_dir/$sample.EFF.dbSNP.combined.snp.vcf  -mutect $mutect_dir/$sample.filtered.vcf  -varscan $varscan2_dir/$sample.snp.Somatic.hc.vcf  -jsm $output_dir/$sample.jsm.tmp.vcf  -sniper $somaticsniper_dir/$sample.vcf  -vardict $output_dir/$sample.vardict.snp.vcf -muse $muse_dir/$sample.vcf  -tbam $tumor_bam -nbam $normal_bam -outfile $output_dir/$sample.ensemble.snp.tsv -mincaller 1\n";
print "$python $somaticseq/SSeq_merged.vcf2tsv.py -ref $genome -myvcf $output_dir/$sample.EFF.dbSNP.combined.indel.vcf -mutect $indelocator_dir/$sample.vcf  -varscan $varscan2_dir/$sample.indel.Somatic.hc.vcf -vardict $output_dir/$sample.vardict.indel.vcf -lofreq $output_dir/$sample.lofreq.indel.vcf -tbam $tumor_bam -nbam $normal_bam -outfile $output_dir/$sample.ensemble.indel.tsv -mincaller 1\n";
...
print "$python $somaticseq/SSeq_tsv2vcf.py -tsv $output_dir/$sample.ensemble.snp.tsv -vcf $output_dir/$sample.ensemble.snp.vcf -pass 0.7 -low 0.1 -tools CGA VarScan2 JointSNVMix2 SomaticSniper VarDict MuSE -all -phred\n";
print "$python $somaticseq/SSeq_tsv2vcf.py -tsv $output_dir/$sample.ensemble.indel.tsv -vcf $output_dir/$sample.ensemble.indel.vcf  -pass 0.7 -low 0.1 -tools CGA VarScan2 VarDict LoFreq -all -phred\n";
