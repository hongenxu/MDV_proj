...
print "$python $somaticseq/SSeq_merged.vcf2tsv.py -ref $genome -myvcf $output_dir/$sample.bina.somatic.indel.vcf -mutect $output_dir/$sample.mutect2.indel.vcf  -varscan $varscan2_dir/$sample.indel.Somatic.hc.vcf -vardict $output_dir/$sample.vardict.indel.vcf -tbam $tumor_bam -nbam $normal_bam -outfile $output_dir/$sample.ensemble.indel.tsv\n";



###prediction
#print "R --no-save --args $model $output_dir/$sample.ensemble.snp.tsv $output_dir/$sample.trained.snp.tsv  <$somaticseq/r_scripts/ada_model_predictor.R \n";
#print "$python $somaticseq/SSeq_tsv2vcf.py -tsv $output_dir/$sample.trained.snp.tsv -vcf $output_dir/$sample.trained.snp.vcf -pass 0.7 -low 0.1 -tools CGA VarScan2 JointSNVMix2 SomaticSniper VarDict MuSE -all -phred\n";
