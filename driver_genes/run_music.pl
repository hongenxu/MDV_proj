...
#working directory
#warnings: this script will create many intermediate files, please choose a directory you have permission to write thousands of files
...
##sample from https://raw.githubusercontent.com/ding-lab/calc-roi-covg/master/data/ensembl_67_cds_ncrna_and_splice_sites_hg19
...
`awk '{print > \$4".tmpfile"}' $output_dir/exons_2bp.bed`;
...
#change back to 1 based start
`awk '{print \$1"\t"\$2+1"\t"\$3"\t"\$4}' $output_dir/exons_2bp.merged.sorted.bed > $output_dir/all_exons.tsv`;

##########create bam file list used by "genome music bmr calc-covg"
...
print CMD "$cmd1\n$cmd2\n$cmd3\n";
close CMD;
#then submit "main_cmd.sh" to computer cluster
